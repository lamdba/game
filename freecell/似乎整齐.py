...
def create():   #常数-队列-对象
	...
	cst.bullet_image5 = pygame.image.load('images/bullet5.png')
	cst.bullet_image6 = pygame.image.load('images/bullet6.png')
	cst.bullet_image7 = pygame.image.load('images/bullet7.png')
	cst.bullet_image8 = pygame.image.load('images/bullet8.png')
...
def make_bullet(n,x,y,d,k = 0):                     #这个版本还没放进去
	...
	if d in [1,-1]:
		...
		a = y
	exec("b{n} = Bullet({n},{x},{y},"
	"{d},{a},{k})".format(n=n,x=x,y=y,d=d,a=a,k=k),
	globals())
	
	def add_bullet(n):
		exec("bullets[{n}]=b{n}".format(n=n),globals())
	add_bullet(count)
