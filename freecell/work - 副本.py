...
def create_shelf(n,x,y): #（可以考虑单独出一个create类）（形如shelf1）#自带全局
	exec("shelf{n} = Shelf({n},{x},{y})".format(n=n,x=x,y=y),globals())       #注意，这里为了方便，用了左下基准的坐标（因为是手动设置位置）
...
def fire_bullet():
	def make_bullet(n,x,y,d):
		exec('b{n} = Bullet({x},{y},{d})'.format(n=n,x=x,y=y,d=d),
		globals())
	def add_bullet(n):
